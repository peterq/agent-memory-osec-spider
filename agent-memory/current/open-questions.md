...
summary: 待用户确认：误删 115.6 万条重爬的投递节奏（工具开发中）；A'/阶段 D 已按答复推进
questions:
  - 当前有哪些待用户确认的问题
load: on-demand
related:
...
- [待确认] 重爬投递节奏：quark 解析走代理池 `quark` 场景（现 ≈6.6 万/h 已有负载），建议额外 ≤3 万/h（≈10/s），111.5 万 ≈ 1.5~2 天；ali 4 万走 `v2aliLoadShare`（20 并发，慢）。可接受？
