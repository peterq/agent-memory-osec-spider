...
- [ ] **P1 十项提案已合入 master 并推送（09-16 12:02），待上线**：删除熔断/检测统一/启动自检/健康告警/CI/密钥/部署回滚/爬虫骨架/搜索配置化 9 条线 + NC-JS 健康页。上线前置（用户）：GitHub 4 个 CI secret；FC 函数 download2oss 配 OSS 环境变量；deploy.sh 新机制先在 osec-restest 演练；熔断器 `breaker.dry_run: true` 观察 ≥1 天再转正；确认 API config.yaml 的 ES 集群是否过期；ListQueue 修复后行为变化。清单 `agent-tasks/2026-09-16-ten-proposals/95-merge-plan.md`。worktree/分支已清理。
- [ ] **P0 P5 阶段 D**：阶段 C 复测 09-15 通过（G1 字面超限用户接受）；灰度 09-15 08:1x 起 30%，09-16 改 `step_every_v3_requests: 500 / step_percent: 2`（两台 res-api 宿主机 `config.yaml` + restart）。半小时邮件由 API `scripts/search_canary_report.sh 30` 发。09-16 白天 35→43%；**10:51 误回落 0%**（v3 错误 1/597 = 深翻页 page=3678 触发 ES max_result_window，v2 同样 500），11:00 用 res2 上的 `search-canary -config config.yaml -set 43 -resume` 恢复；**13:53 第二次误回落**（`kw=test page=1525` 深翻页 5/597），14:00 恢复 53%；**[用户裁定 09-16 14:2x] 深翻页护栏三档**（前端已限前 300 条）：rows=page*pageSize >400 → HTTP 400 报错；>320 → 记 uid 并按 uid(匿名按 ip) 小时计数；1 h 内 >10 次 → 疑似机器人邮件告警。**✅ 14:34~14:37 双机上线**（API `6062fb5`，探测 450→400 / 330→200 记录），14:36 canary 恢复 53%；**09-16 20:4x 灰度到 100%**（21:03 抽样 v3 99.7%、错误 0）；[用户 09-16 21:35 接受建议] 巡检邮件改为每 2 小时一封（`search_canary_report.sh 120`，Monitor 已换）；**09-17 00:18 第四次回落 0%**：v3 2/855 = 匿名请求 `resType=ali/xunlei` 的 500（`ErrInvalidResType`，v2 同样 500，100% 时无 v2 样本可比）→ 非法 resType 改回 400 + 空值归一化 baidu（API `8ed53a8`，01:3x 双机上线，探测 ali→400、空值→200），01:43 恢复 100%→ 观察至 09-19 无异常即 P5 完成，之后 P6（补 v3 detail/fileCtx → 关双写）。附带：res1 `res-api` 14:29:12 被另一会话的 API `./deploy.sh` 销毁未重建，停服 5.5 min 至本会话重建（rollout §17）。A' 存量复检 84,791 条已入队完成。过程原文见快照 archive。
