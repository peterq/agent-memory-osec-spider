...
- [ ] **🔴 P0 lifecycle_checker 误删事故（09-12 22:17 发现）**——正本 SPIDER rollout §15；经验 `lessons/failure-lifecycle_checker误传资源md5导致116万有效资源误删.md`。
  ...
  - ③ **恢复 = 重爬**（用户确认 Mongo 无数据）：导出脚本 `scripts/lc_false_invalid_export.sh` → 投递工具（sonnet 开发中，简报 `agent-tasks/2026-09-12-lc-recrawl/`）限速推 `resourcePreCheck`；范围 **1,156,201**（quark 1,115,264 / ali 40,007 / bnd 930）；投递节奏待用户确认。
