...
updated_at: 2026-09-16T09:02:46+08:00
priority: critical
...
- decisions/decision-2026-09-04-队列v2统一走网关.md | high | 2026-09-12 | SPIDER 跨进程任务投递统一从直连 redis 改为经网关队列 v2；收尾无回滚路径 | 队列v2、queue_task、res_scheduler
...
- procedures/workflow-fc-chrome上线.md | high | 2026-09-13 | fc-chrome 上线链路：本机构建 → docker save/rsync → ACR 推镜像 → s deploy → 共用域名路由 → 健康检查与真实… | fc-chrome、serverless-devs、ACR
...
- procedures/troubleshooting-代理池总览无数据.md | medi | 2026-09-12 | 后台代理池页全 0 时三步定位：网关 dataSource → 上报侧是否部署 → tools 探针 | proxy-admin-check、代理池总览、proxy_admin
...
- lessons/failure-FC实例在WebSocket断开后立即冻结.md | high | 2026-09-13 | 客户端断开 WebSocket 后 FC 立即冻结实例，收尾/写回必须在连接内完成（拦截 Browser.close） | FC 冻结、WebSocket、Browser.close
...
- lessons/failure-resdb的ES端点指向已下线集群.md | high | 2026-09-05 | STORAGE worker 所在 osec-resdb 的 ES 地址指向已下线集群，旧代码静默失败；排查与修法见正文 | osec-resdb、es_endpoint、ES 集群更换
...
- lessons/failure-前端环境默认值写死本地.md | high | 2026-09-08 | 子应用把 gwEndpoint 默认值写死成本地地址导致线上首访连错；默认值必须由 location.hostname 推导 | gwEndpoint、gwAddrs、defaultGwAddr
...
- lessons/failure-握手回包附加字段被传输层丢弃.md | high | 2026-09-12 | 网关握手 403 回包的附加字段被 RTC-gRPC 传输层丢弃；附加信息要走独立通道 | RtcTransport、cancelAllPendingCalls、UnaryCallResponse
...
- lessons/failure-注入脚本用consolelog回传被页面自身替换吞掉.md | high | 2026-09-15 | 页面自身代码会整体替换 window.console.log；注入脚本必须在最开头抓原生引用回传，否则只见一条 start 后沉默 | console.log、CDP、addScriptToEvaluateOnNewDocument
...
- lessons/failure-配置v2二进制无本地回落导致老容器重启即挂.md | high | 2026-09-09 | v2 二进制只认 OSS_CONFIG_URL/LOCAL_CONFIG_PATH 不读宿主机旧配置，老容器重启即挂；需本地回落 | 配置 v2、OSS_CONFIG_URL、config.yaml
...
- knowledge/api-v3-detail-filectx兼容性分析.md | high | 2026-09-15 | lc 索引结构与 v2 detail/fileCtx 查询方式兼容（nested 父子），v3 可复用同一 bndApi | v3 detail、fileCtx、res_lc_all
...
- knowledge/architecture-spider-队列约定.md | medi | 2026-09-12 | SPIDER 队列 v2 的固定队列名、去重键、消费者并发度与通用队列接口约定 | 队列、队列v2、resourcePreCheck
...
- knowledge/reference-github-oauth配置.md | medi | 2026-09-12 | 两个 GitHub OAuth App（prod/dev）的名称、client_id 存放位置、管理页与回调地址约定 | GitHub OAuth App、client_id、redirect_uri
