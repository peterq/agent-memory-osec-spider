...
- `/chrome` 新 query：`extensions=a,b`（策略强装）、`profile=<namespace>/<name>`（持久化；**客户端须先发 `Browser.close` 等响应再断开**；被占直接 409 不等待，客户端退避重试，锁 6 s 无心跳可接管）；504 扩展没装上。`/health` 多了 `cdp3Data/cdp3Temp.writable`、`policyDirWritable`。
