...
updated_at: 2026-09-12T14:40:00+08:00
priority: high
...
summary: 结论——lc 索引结构与 v2 detail/fileCtx 的查询方式兼容（nested filelist、join file、fid/parent、routing 均保留），补 v3 只需换索引 + 去重 + addViews 定位物理索引 + 数字字段容错；无需改 mapping
load: on-demand
related:
...
| `addViews`: 对 `conf.IndexOfResource` 单文档 `Update` | 别名 `res_lc_all` 指向多索引，ES 拒绝单文档写 | ⚠ 需先 `LocateIndex` 定位物理索引再 Update（`locate.go` 注释已预留此用途） |
...
2. **`addViews` 改为 LocateIndex → 物理索引 Update**，遇 404/version_conflict 删缓存重试一次（`lcIdx:<id>` 5min 缓存）。
...
## 不在本分析范围
`query/recommend/suggest/report/likes/dislikes` 同属"暂不提供"清单（PRD §8.1），其中 `report/likes/dislikes` 与 `addViews` 同属单文档 Update，套用第 2 点即可。
