...
## 7. 追加硬规则（2026-09-16 因 deploy-rollback 角色误连生产追加）

12. **任何测试/验收命令都不得出现真实主机名**（`osec-res1/res2/resdb/restest/resngix/jenkins` 及其 IP）：本机 ssh config 对这些名字免密直连生产，「只读命令」也会真的连上去。测试一律用编造的假主机名（如 `faketesthost`）或 `--dry-run`。
